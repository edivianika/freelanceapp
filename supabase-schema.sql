...
-- This would typically be set up in Supabase dashboard or via cron job
-- SELECT cron.schedule('release-expired-ownership', '0 0 * * *', 'SELECT release_expired_ownership();');
...
-- Users can view files for their submissions
...
-- Users can insert files for their submissions
...
-- Users can view status logs for their submissions
...
-- Admins can view all status logs
CREATE POLICY "Admins can view all status logs" ON status_logs
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM users 
            WHERE id = auth.uid() AND role = 'admin'
...
CREATE POLICY "Admins can view override logs" ON override_logs
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM users 
            WHERE id = auth.uid() AND role = 'admin'
